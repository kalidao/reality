...
const CodePreview = dynamic<CodePreviewProps>(() => import('./CodePreview').then((mod) => mod.CodePreview), {
  loading: () => <Box backgroundColor="backgroundSecondary" borderRadius="large" height="48" width="full" />,
})
  ...
  if (live)
    return (
      <PlayroomStateProvider>
        <CodePreview backgroundColor={backgroundColor} code={code} expand={expand} theme={modifiedTheme} />
      </PlayroomStateProvider>
    )
  ...
  return (
    <Highlight {...defaultProps} code={code} language={language} theme={modifiedTheme}>
      ...
      {({ className, style, tokens, getLineProps, getTokenProps }) => (
        <Box as="pre" borderRadius="2xLarge" className={className} padding="6" position="relative" style={style}>

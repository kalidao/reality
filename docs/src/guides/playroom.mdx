...
```tsx live=true
<Stack>
  <Text size="extraLarge">{getState('counter') != null ? getState('counter') : 2}</Text>
  <Stack align="center" direction="horizontal">
    ...
    {getState('counter') != null && getState('counter') !== 2 && (
      <Button disabled={getState('locked')} size="small" variant="tertiary" onClick={() => resetState()}>
        Reset
      </Button>
    )}

    <Button size="small" variant="tertiary" onClick={() => toggleState('locked')}>
